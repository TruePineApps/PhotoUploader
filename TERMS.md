# Terms of Service for Photo-Uploader

**Effective Date:** April 17, 2026

---

### 1. Introduction

This document describes the Terms of Service ("Terms") for **Photo-Uploader** ("the App"),
developed and maintained by **Marcel van Heerwaarden**, trading as **True Pine Apps**
("the Developer").

The App is currently in a **testing phase** and should be treated as beta software.
You acknowledge that the App may contain bugs, errors, or inaccuracies that could
cause failures, data corruption, or loss of data, including photos or metadata.
You are strongly advised to maintain independent backups of all your photos before
using the App and to verify the results of any operation before deleting local
copies.

By installing or using the App, you confirm that you have read and agree to these Terms.
If you do not agree, please do not install or use the App. You will also be asked to confirm
your agreement the first time you launch the App.

**Contact:**
**Marcel van Heerwaarden**, trading as **True Pine Apps**  
KvK: 98723316  
Email: marcel@truepineapps.com  
Full contact details: https://truepineapps.com/en/imprint  

---

### 2. What the App Does

Photo-Uploader is an open-source tool that helps you upload photos from your local
device to your own Google Photos account and organize them into albums based on your local
directory structure.

The App runs entirely on your local device. Your photos are uploaded directly to your own
Google Photos account. No data is transmitted to the Developer or any server controlled by
the Developer. For full details on how personal data is handled, please see the
[Privacy Policy](https://truepineapps.com/photouploader/privacy).

The Developer is only responsible for the App as distributed through the following official
channels:

- [truepineapps.com](https://truepineapps.com)
- [github.com/truepineapps/photouploader](https://github.com/truepineapps/photouploader)

The Developer is not responsible for modified, unofficial, or redistributed versions of the
App obtained from any other source. If you have obtained the App from a different source,
the Developer cannot guarantee its integrity, safety, or compliance with these Terms.

---

### 3. The Apache License and These Terms

The App's source code is made available under the **Apache License, Version 2.0**, which
governs the software itself — including your rights to use, copy, modify, and distribute
the source code. A copy of the Apache License is included with the App.

These Terms are separate from the Apache License. They govern your use of the App as a
running application — your responsibilities as a user, and the Developer's liability toward
you. Nothing in these Terms restricts the rights granted to you by the Apache License.

If there is any conflict between these Terms and the Apache License regarding your rights
to the source code, the Apache License takes precedence.

---

### 4. Access During the Testing Phase

The App is currently in a testing phase. Access requires prior registration with the
Developer. The conditions governing testing access — including the maximum number of users,
access rotation, and how you will be notified — are described in the
[Privacy Policy](https://truepineapps.com/photouploader/privacy).

The Developer makes no commitment regarding the duration of the testing phase or the
continued availability of the App in its current form.

---

### 5. How You May Use the App

To keep things fair and legal for everyone, please make sure that you:

- Only use the App with a Google account that you own or are authorized to use.
- Only upload photos that you own or have the right to upload and organize.
- Only use the App for photos and videos of a personal nature. The App is not intended for storing or serving commercial, enterprise, or non-personal media.
- Comply with [Google's Terms of Service](https://policies.google.com/terms) and
  [Google Photos policies](https://policies.google.com/privacy).
- Do not use the App to upload content that infringes someone else's copyright or other
  intellectual property rights.
- Do not use the App to upload content that is unlawful, harmful, or offensive under
  applicable law.

If the Developer finds that you are not complying with these conditions, your access to the
App may be withdrawn, in addition to any other rights the Developer may have under
applicable law.

---

### 6. No Warranty and Operational Risk

The App is provided **"as is"** and **"as available"**. The Developer does not make any
promises about the App — for example, that it will always work correctly, that it will be
available without interruption, or that it will produce the results you expect.

The App performs automated uploads and organizational actions on your Google Photos library.
While designed to operate safely, such operations may result in unintended outcomes,
including duplicate uploads or incomplete uploads, due to software errors or unforeseen
interactions with third-party services. You acknowledge and accept these risks.

The App only adds content to your Google Photos library — it does not modify or delete any
existing photos or albums. In the event of an unexpected outcome, such as duplicate uploads,
any content created by the App can typically be reviewed and removed directly through Google Photos.

The App interacts with Google Photos through a restricted set of API permissions,
deliberately limited to content created by the App itself. This means the App cannot
observe or verify any side effects that Google's services may have on pre-existing content
in your library. While no such side effects are expected or known, the Developer cannot
guarantee that Google's services will never affect pre-existing content in unforeseen ways.

The App is not intended for use with data that is irreplaceable or where data loss could
result in significant personal or financial harm. You use the App entirely at your own risk.

This is consistent with, and in addition to, the disclaimer of warranties in the Apache
License, Version 2.0.

---

### 7. Liability

The Developer's total liability to you for any claim arising from your use of the App is
limited to the amount you have paid for the App. Since the App is currently free of charge,
this means the Developer's maximum liability is **€0**.

This limitation covers all types of loss, including loss or corruption of photos, loss of
data, and any other direct or indirect damages — to the extent permitted by Dutch and
applicable European Union law.

Under Dutch law, this limitation does not apply to damages caused by the Developer's wilful
misconduct (*opzet*) or gross negligence (*grove schuld*). A court may also reduce or set
aside this limitation if applying it in full would be unacceptable given the circumstances.
Other than in those cases, this limitation applies to the fullest extent permitted by law.

---

### 8. Your Responsibility for Your Data

You are responsible for making sure that:

- You maintain independent backups of your photos at all times. Never rely solely on any single 
  storage location — including Google Photos — as your only copy of important photos.
- You have the right to upload and organize the photos you process with the App.
- You are comfortable with the changes the App will make to your Google Photos library
  before you let it proceed.

---

### 9. Indemnification

By using the App, you agree to cover the Developer's reasonable legal costs and any damages
awarded against the Developer if a third party makes a claim against the Developer arising
from:

- Your violation of someone else's copyright or other intellectual property rights through
  your use of the App.
- Your upload of content that is unlawful in the applicable jurisdiction.
- Your misuse of the App, including use in violation of Section 5 of these Terms.

This indemnification applies only where the claim arises from your own acts or omissions.
It does not apply where the Developer is at fault.

---

### 10. Third-Party Services

The App uses the Google Photos API. Your use of Google Photos is governed by
[Google's Terms of Service](https://policies.google.com/terms) and
[Google's Privacy Policy](https://policies.google.com/privacy). The Developer has no
control over Google's services or policies and is not responsible for any changes Google
makes to its APIs or services that affect how the App works.

---

### 11. Intellectual Property

The App's source code is available under the Apache License, Version 2.0. The name
**True Pine Apps** and associated branding belong to Marcel van Heerwaarden. The Apache
License does not grant you the right to use the Developer's name or branding for
promotional purposes. If you would like to do so, please get in touch first.

---

### 12. Changes to These Terms

If these Terms are updated in a material way, you will be notified in the App and by email
at least **14 days before** the changes take effect — the same way the App handles Privacy
Policy updates. The updated Terms will also be published at
[truepineapps.com/photouploader](https://truepineapps.com/photouploader) with a revised
effective date.

Continued use of the App after the effective date means you accept the updated Terms.

---

### 13. Suspension and Termination

The Developer may suspend or withdraw your access to the App at any time — including if you
do not comply with Section 5, or as part of the testing phase access rotation described in
the Privacy Policy. If your access is withdrawn for non-compliance, you will be notified by
email.

You can stop using the App at any time. You can also revoke the App's access to your Google
account at any time via your
[Google Security Settings](https://myaccount.google.com/permissions).

Termination does not affect any rights or obligations that arose before the termination date.

---

### 14. Governing Law and Jurisdiction

These Terms are governed by the laws of **the Netherlands**.

If you are using the App in a business capacity, any disputes arising from these Terms will
be brought before the competent court in **Utrecht, the Netherlands**.

If you are a consumer, you have the right to bring proceedings either before the courts of
the Netherlands or before the competent court in your country of residence, whichever you
prefer. You also retain the benefit of any mandatory consumer protection provisions of the
law of your country of residence that cannot be excluded by contract, regardless of the
choice of Dutch law in these Terms.

### 14a. Dispute Resolution and Class Action Waiver

Before initiating any formal legal proceedings, you agree to contact the Developer
at marcel@truepineapps.com to attempt to resolve the dispute informally. The
Developer will make reasonable efforts to respond to any complaint within 30 days.

To the fullest extent permitted by applicable law, you agree that any claim arising
from your use of the App must be brought on an individual basis only. You waive any
right to bring or participate in a class action, collective action, or representative
proceeding against the Developer.

This waiver does not apply where prohibited by mandatory law in your country of
residence.

---

### 15. Severability

If any part of these Terms turns out to be invalid or unenforceable under applicable law,
that part will be adjusted to the minimum extent needed to make it enforceable or removed
if adjustment is not possible. The rest of the Terms remain in effect.

---

### 16. Contact

For questions about these Terms, please contact:

**Marcel van Heerwaarden**, trading as **True Pine Apps**  
KvK: 98723316  
Email: marcel@truepineapps.com  
Full contact details: https://truepineapps.com/en/imprint