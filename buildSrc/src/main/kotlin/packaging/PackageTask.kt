...
abstract class PackageTask : DefaultTask() {
     ...
     * NOTICES is a file generated by Gradle task 'generateLicenseReport'. Since it is shown on the
     * LicenseScreen, it is also checked in.
     */
    protected fun copyLicenseFiles(targetDir: File) {
        targetDir.mkdirs()

        licenseSourceFile.asFile.get().copyTo(
            target = File(targetDir, "LICENSE"),
            overwrite = true
        )
        ...
        sharedResourceDir.asFile.get().listFiles()?.filter {
            (it.name.startsWith("TERMS") || it.name.startsWith("PRIVACY"))
                    && it.extension == "md"
        }?.forEach { file ->
            copyResourceFile(file.name, targetDir)
        }
    }
